...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    if (data.user) await ensureProfile({ id: data.user.id, email: data.user.email ?? '', user_metadata: data.user.user_metadata });
  ...
  return (
    <AuthLayout
      ...
      footer={
        <>
          New to SkillQuest?{' '}
          <Link href="/register" className="font-medium text-primary hover:text-glow-primary">
            Create an account
          </Link>
        </>
      }
    >
      <form onSubmit={handleEmailLogin} className="space-y-4">
        ...
        <Button type="submit" className="w-full gap-2" disabled={submitting}>
          ...
          {submitting ? 'Entering the realm...' : 'Sign In'}
        </Button>
      </form>
